...
- **Pilot before production** — method simulations and one-off tests start in a private workspace; promote to `dofiles/` only by intent
...
- **Stata version on this machine:** Stata 18 (`D:\Program Files\Stata18\StataMP-64.exe`).
  Added to PATH so `scripts/run_stata.sh` resolves it.
- **Python on this machine:** Anaconda at `D:\ProgramFiles\anaconda3\python.exe`.
  Conda is at `D:\ProgramFiles\anaconda3\Scripts\conda.exe`.
- **Pin Stata version** at top of every do-file: `version 18`
...
- **No root logs** — move Stata console transcripts into the relevant project `logs/` folder
